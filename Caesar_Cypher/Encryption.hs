...
normalise text = map (toUpper) $ filter (isAlphaNum) text
...
reverseKey keymap = map (\(a,b) -> (b,a)) keymap

contains :: String -> String -> Bool
contains [] _ = True
contains _ [] = False
contains str1 str2
    | isPrefixOf str1 str2 = True
    | otherwise = contains str1 (tail str2)


countFreqs text = zip (map (head) grp) (map (length) grp)
    where grp = group $ sort text
